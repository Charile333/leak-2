...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const findings = await listScheduledFindings({ userEmail, scanType: 'code_leak' });
    ...
    console.error('[api/code-leak/findings] failed:', error);
    return sendJson(res, 500, {
      success: false,
      error: 'Code leak findings failed',
